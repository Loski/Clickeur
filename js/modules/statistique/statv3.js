    ...
    .controller('statsController', [ '$scope', 'questionWithStatistique','statistiqueRepository','questionRepository','$timeout','$state' , 'modalService', '$window',function($scope, questionWithStatistique,statistiqueRepository,questionRepository,$timeout,$state,modalService,$window){
        ...
        $scope.updateChart = function()
        {
            ...
            /****/
                ...
                for(var index in $scope.question.propositions)
                {
        ...
        var update = function(auto)
        {
           statistiqueRepository.getQuestionWithStatistique($scope.question.id).then(function successCallback(success){
				...
				for(var i = 0; i < $scope.question.propositions.length; i++){
					$scope.stat = $scope.question.propositions[i].stat;
					var proposition = proposition[i];
					if(proposition.verdict == 1){
						$scope.indice_good.push(number);
					}
					for(var j = 0; j < stat.tour[tour_actuel].users.length; j++){
						var indice = $scope.findStudent(stat.tour[tour_actuel].users[j].id);
						if(indice < 0){
							$scope.students.push({
				                num_etu:stat.tour[tour_actuel].users[j].username,
				                firstName:stat.tour[tour_actuel].users[j].firstName,
				                lastName:stat.tour[tour_actuel].users[j].lastName,
				                proposition:[],
				                proposition_juste:[]
				            });
				            indice = $scope.students.length - 1;
				        }
						else{
							$scope.students[indice].proposition.push(proposition.number),
				            $scope.student[indice].proposition_juste.push(proposition.verdict)
						}
					}
				}

				for(var i = 0; i < $scope.students.length; i++){
					if($scope.students[i].proposition.length == response_count){
						for(var j = 0; j < indice_good.length; j++){
							// Même ordre d'insertion so..
							if($scope.students[i].proposition[j].number != indice_good[j]){
								$scope.students[i].isGood = false;
								break;
							}
							$scope.students[i].isGood = true;
						}
					}else{
						$scope.students[i].isGood = false;
					}
